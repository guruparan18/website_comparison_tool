...
app.secret_key = (
    "your_very_secret_random_string_here"  # Ensure this is a strong, unique key
)
    ...
    for site_name_folder in os.listdir(base_screenshot_dir):
        ...
        if os.path.isdir(abs_site_path):
            ...
            for timestamp_folder in os.listdir(abs_site_path):
                ...
                if os.path.isdir(abs_timestamp_path) and os.path.exists(
                    os.path.join(abs_timestamp_path, "crawled_data.json")
                ):
                    timestamps.append(timestamp_folder)
            if timestamps:
                grouped_crawls[site_name_folder] = sorted(
...
def index():
    ...
    if request.method == "POST":
        ...
        available_crawls_for_template = list_available_crawls_grouped(
        ...
        s1_name_sanitized = (
            s1_domain.replace(".", "_") if s1_domain else "website1_default"
        )
        s2_domain = crawler.get_domain(form_url2)
        s2_name_sanitized = (
            s2_domain.replace(".", "_") if s2_domain else "website2_default"
        )

        site1_info = {"site_name_sanitized": s1_name_sanitized}
        if (
            selected_existing_crawl1
        ):  # A specific folder path like 'site_name_folder/timestamp'
        ...
        thread = threading.Thread(
            target=run_comparison_workflow,
            args=(form_url1, site1_info, form_url2, site2_info, new_run_timestamp),
        )  # Pass timestamp for new crawls
        thread.start()
...
def run_comparison_workflow(url1, site1_info, url2, site2_info, new_run_timestamp):
    ...
    try:
        ...
        if site1_info["action"] == "crawl":
            ...
            site1_output_dir = os.path.join(
                app.config["UPLOAD_FOLDER"],
                site1_info["site_name_sanitized"],
                new_run_timestamp,
            )
            os.makedirs(site1_output_dir, exist_ok=True)
            print(
                f"Starting FRESH CRAWL for Website 1 (Legacy): {url1} -> saving to {site1_output_dir}"
            )
            pages1_data = crawler.crawl_website(
                url1, site1_output_dir, is_modern_site=False
            )  # output_dir_base is where it saves images
            if pages1_data:
                save_crawled_data(
                    pages1_data, site1_output_dir
            ...
            full_load_path = os.path.join(
                app.config["UPLOAD_FOLDER"], site1_info["path"]
            )
            print(
                f"LOADING existing data for Website 1 (Legacy) from: {full_load_path}"
            )
        ...
        if site2_info["action"] == "crawl":
            site2_output_dir = os.path.join(
                app.config["UPLOAD_FOLDER"],
                site2_info["site_name_sanitized"],
                new_run_timestamp,
            )
            os.makedirs(site2_output_dir, exist_ok=True)
            print(
                f"Starting FRESH CRAWL for Website 2 (Modern): {url2} -> saving to {site2_output_dir}"
            )
            pages2_data = crawler.crawl_website(
                url2, site2_output_dir, is_modern_site=True
            )
            if pages2_data:
            ...
            full_load_path = os.path.join(
                app.config["UPLOAD_FOLDER"], site2_info["path"]
            )
            print(
                f"LOADING existing data for Website 2 (Modern) from: {full_load_path}"
            )
        ...
        comparison_results = comparator.compare_pages(
            pages1_data, pages2_data, url1, url2
        )
