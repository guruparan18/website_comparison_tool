...
def index():
    ...
    # Retrieve last submitted URLs from session for GET requests or to pre-fill form
    ...
    if request.method == 'POST':
        ...
        # Store submitted URLs in session immediately to retain them
        session['last_url1'] = url1_from_form
        session['last_url2'] = url2_from_form
        # Update local variables for current request processing
        ...
        if not form_url1 or not form_url2:
            return render_template('index.html',
                                   ...
                                   form_url1=form_url1, # Pass back to template
                                   form_url2=form_url2) # Pass back to template
        ...
        # Reset previous results for this new submission
        comparison_results = []

        timestamp = datetime.datetime.now().strftime("%Y%m%d%H%M%S")
        repo_path = app.config['UPLOAD_FOLDER'] # Use app.config
        ...
        # Pass the correct form_url1, form_url2 to the thread
        thread = threading.Thread(target=run_comparison_workflow, args=(form_url1, form_url2, path1, path2))
        thread.start()

        return redirect(url_for('index')) # Redirect to GET to show progress

    # For GET request, render with URLs from session (or empty if first visit)
    return render_template('index.html',
                           results=comparison_results,
                           crawl_status=crawl_status,
                           form_url1=form_url1,
