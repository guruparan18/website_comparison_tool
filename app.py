...
def normalize_url_for_matching(
    url,
):  # This function seems unused in app.py, ensure it's in comparator or crawler if needed there
...
def index():
    ...
    if request.method == "POST":
        ...
        thread = threading.Thread(
            target=run_comparison_workflow, args=(form_url1, form_url2, path1, path2)
        )
        thread.start()
