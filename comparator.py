...
def analyze_pixel_and_structural_differences(
    ...
    try:
        ...
        _, threshold_img = cv2.threshold(abs_diff_img, 30, 255, cv2.THRESH_BINARY)

        total_pixels = threshold_img.shape[0] * threshold_img.shape[1]
        diff_pixels = cv2.countNonZero(threshold_img)
        analysis_results["diff_percent"] = (
            (diff_pixels / total_pixels) * 100 if total_pixels > 0 else 0
        )

        # 3. Save Visual Difference Image (threshold_img)
        if diff_image_save_rel_path:
            abs_save_path = os.path.abspath(diff_image_save_rel_path)
            try:
                os.makedirs(os.path.dirname(abs_save_path), exist_ok=True)
                cv2.imwrite(abs_save_path, threshold_img)
    ...
    for i, norm_path in enumerate(all_normalized_paths):
        ...
        if result_entry["img1_full"] and result_entry["img2_full"]:
            ...
            if analysis["ssim_score"] is not None:
                print(
                    f"  SSIM: {analysis['ssim_score']:.4f} ({classification['text']}), Diff %: {analysis['diff_percent']:.2f}%"
                )
            else:
